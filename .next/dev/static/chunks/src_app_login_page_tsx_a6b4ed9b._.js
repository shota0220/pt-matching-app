...
function LoginPage() {
    ...
    const handleLogin = (role)=>{
        ...
        // 2. もしバリデーションに失敗したら
        if (!result.success) {
            // ★ errors[0] を使わず、flatten() でエラーメッセージを取り出す
            const fieldErrors = result.error.flatten().fieldErrors;
            // 最初に見つかったエラー（email か password）を表示
            const message = fieldErrors.email?.[0] || fieldErrors.password?.[0] || "入力内容に誤りがあります";
            alert(message);
            return; // ★ ここで処理を止めるので、次の画面には絶対に行きません
        }
        // 3. バリデーションに成功した場合のみ、以下の処理が実行される
        if (role === "user") {
            router.push("/search");
        } else {
