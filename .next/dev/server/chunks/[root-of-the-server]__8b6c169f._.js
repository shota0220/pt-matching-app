...
"[project]/src/app/api/chats/list/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        ...
        // 1. まずチャット一覧を取得（リレーションは使わず、IDだけ）
        const chatRooms = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].chat.findMany({
        ...
        if (role === "THERAPIST") {
            // PT → 相手は user
            const userIds = Array.from(new Set(chatRooms.map((c)=>c.userId)));
            ...
            // USER → 相手は therapist
            const therapistIds = Array.from(new Set(chatRooms.map((c)=>c.therapistId)));
            const therapists = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].therapist.findMany({
                where: {
                    id: {
                        in: therapistIds
                    }
            ...
            const therapistMap = new Map(therapists.map((t)=>[
                    t.id,
                    t
                ]));
            const formatted = chatRooms.map((room)=>{
...
//# sourceMappingURL=%5Broot-of-the-server%5D__8b6c169f._.js.map